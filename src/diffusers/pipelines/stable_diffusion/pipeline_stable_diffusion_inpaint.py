    ...
    def prepare_latents(
        ...
        if return_image_latents or (latents is None and not is_strength_max):
            ...
            if image.shape[1] == 4:
                ...
                image_latents = self._encode_vae_image(image=image, generator=torch.Generator(device="cuda").manual_seed(42))
            image_latents = image_latents.repeat(batch_size // image_latents.shape[0], 1, 1, 1)

        if latents is None:
            noise = randn_tensor(shape, generator=torch.Generator(device="cuda").manual_seed(42), device=device, dtype=dtype)
